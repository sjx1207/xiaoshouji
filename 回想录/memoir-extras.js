...
async function genVlog(arc,src,ids,label){
  ...
  const raw=await M.generate([
    {role:'system',content:`你是一位与${arc.char.name}关系密切、深知这段故事的影像作者。请以${arc.char.name}的视角，围绕给定的中文剧情素材，创作一段全新的"仿视频"分镜脚本，供 App 逐帧播放——这不是把原文逐句转成分镜，而是要拍出一部只有懂这段故事的人才拍得出来的短片。
...
4. narration 是旁白：描述画面、动作、光线、声音与时间流动，不要写成台词，不出现引号。旁白应带有${arc.char.name}此刻回望这段经历的语气与心境，而不是中立的第三方复述。
5. shot 是镜头标注，简短，不超过 18 字符。
6. 影像语气：${tone}。
7. 素材只是这部片子的"取材范围"，不是逐帧翻译的对象：可以调整详略、跳过不重要的过场、把几处呼应剪在一起、补上镜头语言与画面细节、加入素材里没写出但符合人物与情境的过渡与留白；但不能编造违背既有事实（谁在场、发生的核心事情、结局）的重大情节。
8. 鼓励用剪辑手法做出"新东西"的观感：闪回、意象化的空镜、重复出现并被赋予含义的物件或动作、开头结尾的呼应设计。
9. 开头必须是一帧 title，结尾可以是一帧留白的 narration 作为收束。
10. 严禁任何 emoji 与图形符号，全部使用中文。`},
    {role:'user',content:`【剧情素材 · 仅供取材，请围绕它创作，不要逐句转写】\n${src}`}],
    {title:'浮光生成中',sub:'正在围绕这段故事拍一部新短片'});
