...
let genT=null, genStart=0, genChars=0, genGlyphT=null;
...
async function chat(messages, opt){
  ...
  const run=async(payload,stream)=>{
  ...
  while(M.cfg.retryOnCut && out.finish==='length' && guard<2){
    ...
    const cont=await run(Object.assign({},body,{messages:msgs}),false);
    text+=cont.text||'';
...
body{font-family:'Noto Sans SC',-apple-system,'PingFang SC',sans-serif;color:#26262d;
  font-size:14px;line-height:2;letter-spacing:.02em;padding:22px 20px 24px}
...
function rp(){parent.postMessage({__m:'h',h:Math.max(document.body.scrollHeight,document.documentElement.scrollHeight)},'*');}
var t;function ping(){clearTimeout(t);t=setTimeout(rp,40);}
