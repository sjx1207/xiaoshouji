...
F.home.genSheet = (w) => {
  ...
  let charPosts = 0, extra = '';
  const MAX = 20;
  F.sheet({
    title: w.name, sub: '选择想看的类型与篇数，可以自由组合', foot: true, cls: 'gen-sheet',
    render(api) {
      ...
      const draw = () => {
        ...
        api.body.innerHTML = `
          ...
          ${F.state.charProfiles.length ? `<div class="group-title">角色是否参与本次生成</div>
        ...
        api.foot.querySelector('.gs-go').onclick = () => {
          ...
          const jobs = bag.map((type, index) => ({
            index, type, worldId: w.id, extra: extra.trim(), chars: Object.keys(sel), roles: { ...sel },
            authorChar: charSlots.has(index) && authors.length ? authors[(ai++) % authors.length] : null, status: 'wait'
          }));
          api.close();
...
F.home.runGen = (w, jobs) => {
  F.nav.push(async (page, api) => {
    ...
    page.innerHTML = `
      ...
      <div class="page-scroll"><div class="gp">
        ...
        <div class="gp-jobs" id="gpJobs">${jobs.map((j, i) => `
          <div class="gp-job" data-j="${i}"><span class="gs-glyph g-${j.type}">${F.I(F.TYPES[j.type].icon)}</span>
            <div class="gj-main"><b>${F.TYPES[j.type].name}</b><small>${j.authorChar ? F.esc(F.charProfile(j.authorChar).nickname) + ' 发布' : '网友发布'}</small></div>
    ...
    const res = await F.gen.run(jobs, ({ i, job, phase, txt, error }) => {
      const n = F.TYPES[job.type].name;
      if (phase === 'start') {
        $('#gpTitle').textContent = `正在撰写第 ${i + 1} 篇 · ${n}`; $('#gpSub').textContent = '连接模型…'; setJob(i, 'writing');
        page.querySelector(`[data-j="${i}"]`).scrollIntoView({ block: 'nearest', behavior: 'smooth' });
      }
      if (phase === 'delta') {
      ...
      if (phase === 'done') { finished++; setJob(i, 'done', job.post ? '' : undefined); const a = F.person(F.post(job.postId).author); page.querySelector(`[data-j="${i}"] small`).textContent = `${a.nickname} · 已完成`; }
    ...
    $('#gpSub').textContent = ok ? `${ok} 篇内容已经发布到「${w.theme || w.name}」${fail ? `，${fail} 篇失败` : ''}` : '请检查 API 设置或稍后重试';
    ...
    const v = page.querySelector('[data-view]'); if (v) v.onclick = () => { F.home.cat = w.id; F.home.shown = 20; F.nav.popAll(); F.tabs.go('home'); setTimeout(() => F.tabs.refresh(), 60); };
    ...
    page.querySelector('[data-again]').onclick = () => { api.close(); setTimeout(() => F.home.genSheet(w), 400); };
  });
};
...
F.dm.thread = (peer, create = true) => {
  ...
  if (!t && create) { t = { id: F.uid('dm'), identityId: me, peer, messages: [], unread: 0, updatedAt: Date.now() }; F.state.dms.push(t); }
  return t;
};
F.dm.openList = (at) => {
  F.nav.push((page, api) => {
    const draw = () => {
      const ts = F.state.dms.filter(t => t.identityId === F.state.activeIdentityId && t.messages.length).sort((a, b) => b.updatedAt - a.updatedAt);
      page.innerHTML = `${F.topbar('私信', { sub: '以 ' + F.me().nickname + ' 的身份', right: `<button class="gorb lg" data-new>${F.I('edit')}</button>` })}
        <div class="page-scroll"><div class="dm-list">
          ${ts.length ? ts.map(t => { const p = F.person(t.peer); const last = t.messages[t.messages.length - 1]; return `
            <button class="dm-item" data-t="${t.id}">${F.avatarHTML(p, 52)}
              ...
              <p class="dm-last">${last.from === 'me' ? '我：' : ''}${F.esc(last.text)}</p></div>
              ${t.unread ? `<i class="dm-dot">${t.unread}</i>` : ''}</button>`; }).join('') :
            `<div class="empty"><div class="e-art lg" style="--lg-r:30px">${F.I('mail')}</div><h4>还没有私信</h4><p>在任何博主或角色的主页点「私信」，<br>或点右上角选择一位你关注的人开始聊天。</p></div>`}
        </div></div>`;
      F.bindBack(page, api);
      ...
      page.querySelector('[data-new]').onclick = async () => {
        ...
        const uniq = people.filter((r, i) => people.findIndex(x => F.refEq(x, r)) === i);
...
F.dm.openChat = (peer) => {
  ...
  F.nav.push((page, api) => {
    ...
    page.innerHTML = `
      <header class="topbar"><div class="tb-side"><button class="gorb lg" data-back>${F.I('back')}</button></div>
        <button class="tb-title dm-peer" data-peer>${F.avatarHTML(p, 30)}<span><b>${F.esc(p.nickname)}${F.isVerified(p) ? F.vbadge(p.kind !== 'char') : ''}</b><small>@${F.esc(p.handle)}</small></span></button>
    ...
    const draw = (typing) => {
      let last = 0;
      box.innerHTML = (t.messages.length ? '' : `<div class="dm-hello">${F.avatarHTML(p, 64)}<b>${F.esc(p.nickname)}</b><small>${F.esc(p.bio || p.persona || '')}</small><p>发出消息后，点左下角的光点按钮，TA 才会回复你。</p></div>`) +
        t.messages.map(m => { const gap = m.time - last > 600000; last = m.time; return `${gap ? `<div class="dm-time">${new Date(m.time).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</div>` : ''}<div class="bub ${m.from}">${m.from === 'peer' ? F.avatarHTML(p, 30) : ''}<p>${F.esc(m.text).replace(/\n/g, '<br>')}</p></div>`; }).join('') +
        (typing ? `<div class="bub peer">${F.avatarHTML(p, 30)}<p class="typing"><span class="cm-typing"><i></i><i></i><i></i></span></p></div>` : '');
    ...
    page.querySelector('[data-ask]').onclick = async e => {
      ...
      try {
        ...
        for (const m of msgs) { await F.sleep(500 + Math.min(1400, m.length * 30)); t.messages.push({ from: 'peer', text: m, time: Date.now() }); draw(msgs.indexOf(m) < msgs.length - 1); }
    ...
    page.querySelector('[data-menu]').onclick = async () => {
      const v = await F.menu(p.nickname, [{ label: '查看主页', value: 'home', icon: 'users' }, { label: '清空聊天记录', value: 'clear', icon: 'trash', danger: true }]);
